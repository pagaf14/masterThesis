...
for iter in range(0,itmax):
    if iter==0:
        ...
        os.system("mpirun -np 6 SU2_CFD inv_wedge_ROE.cfg")
    else:
        # Run the simulation with restart
        os.system("mpirun -np 6 SU2_CFD inv_wedge_ROE_restart.cfg")
    ...
    for ii in nodeTags_py:
        if err_nodes[ii] < tau_c_nodes:
            newDist[ii] = min( [1.2*nodeAvgDist[ii], l_max] )
        elif err_nodes[ii] < tau_r2_nodes and err_nodes[ii] > tau_r1_nodes:
    ...
    if iter==0:
        gmsh.write("adapt_0.msh")
    elif iter==1:
        gmsh.write("adapt_1.msh")
    elif iter==2:
        gmsh.write("adapt_2.msh")
    elif iter==3:
        gmsh.write("adapt_3.msh")
    elif iter==4:
        gmsh.write("adapt_4.msh")
    elif iter==5:
        gmsh.write("adapt_5.msh")
    elif iter==6:
        gmsh.write("adapt_6.msh")
    elif iter==7:
        gmsh.write("adapt_7.msh")
    elif iter==8:
        gmsh.write("adapt_8.msh")
    gmsh.finalize()
...
# Run the simulation with restart
os.system("mpirun -np 4 SU2grad_CFD inv_wedge_ROE_restart.cfg")
